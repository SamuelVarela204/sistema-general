...
/**
 * Asegurar existencia de tabla para configuración global
 */
function asegurarTablaGlobalSettings($con)
{
    ...
    $check = mysqli_query($con, 'SELECT id FROM global_settings WHERE id = 1 LIMIT 1');
    if (!$check || mysqli_num_rows($check) === 0) {
        mysqli_query($con, 'INSERT INTO global_settings (id) VALUES (1)');
    }
...
/**
 * Guardar fondo global (BLOB) y su mime
 */
function setGlobalBackground($con, $blob, $mime)
{
    if (!asegurarTablaGlobalSettings($con)) return false;

    $null = null;
    $stmt = mysqli_prepare($con, 'UPDATE global_settings SET glob_wall = ?, glob_mime = ? WHERE id = 1');
    if (!$stmt) return false;

    // bind_param requires variables; use 'b' for blob and send_long_data
    mysqli_stmt_bind_param($stmt, 'bs', $null, $mime);
...
/**
 * Obtener fondo global: devuelve ['blob'=>..., 'mime'=>...] o null
 */
function getGlobalBackground($con)
{
    if (!asegurarTablaGlobalSettings($con)) return null;

    $stmt = mysqli_prepare($con, 'SELECT glob_wall, glob_mime FROM global_settings WHERE id = 1 LIMIT 1');
    if (!$stmt) return null;
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
    ...
    if ($row && !empty($row['glob_wall'])) {
        return ['blob' => $row['glob_wall'], 'mime' => $row['glob_mime']];
    }
    return null;
}

/**
 * Eliminar el fondo global almacenado en la tabla
 */
function deleteGlobalBackground($con)
{
    if (!asegurarTablaGlobalSettings($con)) return false;
    $stmt = mysqli_prepare($con, 'UPDATE global_settings SET glob_wall = NULL, glob_mime = NULL WHERE id = 1');
    if (!$stmt) return false;
    $ok = mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
    return (bool)$ok;
