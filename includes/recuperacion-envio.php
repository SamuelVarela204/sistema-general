...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['recuperar'])) {
    ...
    $baseUrl = rtrim(dirname(dirname($_SERVER['SCRIPT_NAME'])), '/\\');
    $enlace = $baseUrl . '/index.php?page=restablecer&token=' . urlencode($token);

    $mailConfig = require __DIR__ . '/../config/mail.php';
    if (empty($mailConfig['host']) || str_contains($mailConfig['username'], 'tu-correo') || str_contains($mailConfig['password'], 'tu-contraseña')) {
        error_log('[Recuperación] Configuración SMTP incompleta en config/mail.php');
        mysqli_close($con);
    ...
    try {
        ...
        error_log('[Recuperación] Error al enviar correo: ' . ($mail->ErrorInfo ?? 'n/a') . ' | ' . $e->getMessage());
        if (!empty($debugOutput)) {
            error_log('[Recuperación] SMTP debug: ' . $debugOutput);
        }
        mysqli_close($con);
        redirigirAlInicio('page=recuperacion&error=mail_error');
    }
